 ...
 * My release TWI only:
 * empty sketch: 552/7
 * +arhat_twi.h: 1240/22 (+688/+15)
 * +вывод строки(14): 1362.38 (+122/+16)
 *
 * Wiring TWI only:
 * Empty loop = 1548/110
 * write string(14) = 1774/126

#include "arhat.h"
#include "arhat_twi.h"
void setup() { twiSetup(100000, TWI_READY|TWI_SEND_STOP); }
void loop() { char *_s = "Hello, Andrey!"; twiWrite(0x27, (uint8_t *)_s, 14); }
*/
/*
 * Мой релиз + LCD: 1838/38 (+598/+14)
 * +вывод строки: 1946/50 (+108/+14)
 
#include "arhat.h"
#include "lcd1602.h"
void setup() { lcdSetup(0x27, 16, 2, 1); }
void loop()  { lcdWrite("Hello, Andrey!", 14); }
*/
/*
 * arhat+тип.LCD: 3116/235
 * +вывод строки(14): 3186/251 (+70/+16)
 * только Wiring, без arhat.h: 3326/253
...
void setup() { lcd.init(); }
void loop() { lcd.print("Hello, Andrey!"); }
*/
/*
void setup() {}
void loop() {}
...
#define LCD_I2C_SPEED    1000  // @TODO Можно поиграться с этим .. сколько там регистр вытянет на самом деле..
...
//  twi_init();
  ...
  Serial.println("");
  Serial.print("TWCR=");
  Serial.print(TWCR, 10);
  Serial.print(", TWBR=");
  Serial.print(TWBR, 10);
  Serial.print(", TWSR=");
  Serial.print(_SFR_BYTE(TWSR), 10);
  Serial.println("");
  #define TWI_SETUP "lcd1602.h"                     // фиксим, что I2C инициализируется этой библиотекой
#else
  twiSpeed(LCD_I2C_SPEED);                          // мало ли как он был уже запущен .. lcdSetup() работает так!
  twiMode = TWI_SEND_STOP;
#endif
