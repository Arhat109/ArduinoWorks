...
#define _twiWaitReady()             {while((TWI_READY & twiMode)==0); twiMode &= ~TWI_READY;}
...
#define LCD_WAIT_BOOT2      500  // (по 250нсек) >100мксек пауза третьей 0x30
#define LCD_WAIT_1          190  // тиков Т0: 4мксек - мало! (>37мксек пауза на команду "в среднем")
...
void setup() {
  Serial.begin(115200);
    lcdSetup(0x27, 16, 2, 1);
    {
      ...
      lcdWrite("Hello, Arhat!   ", 16);
      Serial.print(micros()-start, DEC);
    }
    lcdSetCursor(0,1);
    lcdWrite("Bye, Bye..", 10);
}
