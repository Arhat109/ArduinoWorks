...
//            tsc_simple( &srvSonic.ctrl, srvSONIC_next, 0);          // Крутим серву датчика
            ...
            motor[m0_id].speed = 255;
            motor[m3_id].speed = 230;
            motorForward(m0_id);
            motorForward(m3_id);
            brain.ctrl.timeout = 976;
            brain.state = BRAIN_WAIT;
            break;

        case BRAIN_WAIT:
            motorStop(m0_id);
            motorStop(m3_id);
            brain.ctrl.timeout = 3000;
            brain.state = BRAIN_LEFT;
            break;
        ...
        case BRAIN_LEFT:
            ...
            motorBack(m0_id);
            motorForward(m3_id);
            brain.ctrl.timeout = 1800;  // 0.2 sec.
